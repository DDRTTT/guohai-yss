...
import {
  ...
  deleteTableAPI,
  revokeTableAPI,
} from '@/services/capitalVerificationLetter';
import { routerRedux } from 'dva/router';

export default {
  namespace: 'capitalVerificationLetter',
  state: {
    name: 'capitalVerificationLetter',
    dataSource: [],
    total: 0,
    loading: false,
    proTypeList: [],
  ...
  effects: {
    ...
    *getTrusteeEnum({ payload }, { call, put }) {
      const response = yield call(trusteeEnum);
      if (response && response.status === 200) {
        yield put({
          type: 'updateTrusteeEnum',
          payload: {
            trusteeList: response.data,
          },
        });
      }
    },
  ...
  reducers: {
    ...
    updateTrusteeEnum(state, action) {
      return {
        ...state,
        trusteeList: action.payload.trusteeList,
      };
    },
