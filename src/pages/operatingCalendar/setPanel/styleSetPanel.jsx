...
import { Tabs, Spin, Menu, Layout, Row, Col, Button, Input, Modal } from 'antd';
import { connect } from 'dva';
import { cloneDeep } from 'lodash';
import { CheckColorItem, SetPanelLayout } from './strategyComponent';
...
class StyleSetPanel extends React.Component {
  // 用于提交用的数据
  submitData = [];
  // 当前模版的名称
  currentTemplateName = '';
  ...
  // 添加模版
  add = () => {
    const { firstActivety, dataSourceList } = this.state;
    this.props
      .dispatch({
                ...
                <TabPane tab={pane.name} key={index}>
                  ...
                  <Layout>
                    ...
                    <Layout style={{ backgroundColor: '#fff' }}>
                      <Row
                        //   className={setPanelStyles.generalTitle}
                        type="flex"
                      ...
                      >
                        <h4>颜色设置</h4>
                        <div>{/* <span style={{ marginRight: '20px' }}>重置颜色</span> */}</div>
                      </Row>
                      <Content>
...
export default connect(styleSet)(StyleSetPanel);
