 ...
 * 设置页面的导航
 */
export const SetPanelLayout = props => {
  const { currentTitle, rightNode = <></>, children } = props;
  return (
    <Card title={currentTitle} extra={rightNode}>
      <Content className={setPanelStyles.generalContent}>{children}</Content>
    </Card>
  );
};
/**
...
export const CheckColorItem = props => {
  ...
  return (
    <div className={setPanelStyles.pickColorItem}>
      <span>{name}</span>
      <input
        type="color"
...
const TemplateItem = props => {
  ...
  return (
    <div style={{ marginRight: '10px' }}>
      <div>
        <div>
          <div style={{ backgroundColor: list[0]?.color, height: '30px', width: '150px' }}></div>
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
