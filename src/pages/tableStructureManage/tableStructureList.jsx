...
class Index extends Component {
    ...
    this.props.form.validateFields(
      ...
      (err, vals) => {
        ...
        Object.keys(vals)?.forEach(k => {
          if (vals[k] === 0 || vals[k]) {
            // 防止：是否系统级别选择否（number 0）被过滤走，应保留
            conditions.push({
              name: k,
              operator: ['attributeName', 'bizDataName'].includes(k) ? 'LIKE' : 'EQ',
              value: vals[k],
            });
          }
        });
  ...
  advancSearch = (vals, isResetCurrent) => {
    // 高级搜索-查询：isResetCurrent 为 true，重置 current 为 1
    if (!vals) {
      vals = {};
    }
    ...
    Object.keys(vals)?.forEach(k => {
      if (vals[k]) {
        conditions.push({
          name: k,
          operator: ['masterTableDesc', 'masterTableName'].includes(k) ? 'LIKE' : 'EQ',
          value: vals[k],
        });
      }
    });
  ...
  setfieldModal = (fieldTitle, isBatch, record) => {
    // 修改表字段
    isRequired = true;
    selNumber = 0;
    const { selectedRows, data } = this.state;
    this.setState(
      { fieldTitle, isBatch, record, fieldVisible: true, fieldLoading: true, checkColumnData: [] },
      () => {
        let id = '';
        if (isBatch) {
        ...
        .then(res => {
          if (res) {
            ...
            newData?.forEach(nd => {
              res?.forEach(r => {
                res = res.filter(
                  key => key?.columnName?.toUpperCase() !== nd?.columnName?.toUpperCase(),
                );
              });
            });
            res?.forEach(item => {
    ...
    this.setState({ elementsLoading: true }, () => {
      this.props
        .dispatch({
          ...
          payload: {
            conditionsOr: [
              { name: 'attributeName', operator: 'LIKE', value: val },
              {
                name: 'bizDataName',
                operator: 'LIKE',
                value: val,
            ...
            conditions: [
              { name: 'orgId', operator: 'EQ', value: record.orgId },
              {
                name: 'sysId',
                operator: 'EQ',
                value: record.sysId,
  ...
  render() {
    ...
    const tableStructureColumns = [
      ...
      {
        title: '是否继承基类',
        dataIndex: 'extendBase',
        key: 'extendBase',
        render: text => (+text === 1 ? '是' : '否'),
      },
      {
        title: '是否为中间表',
        dataIndex: 'middleTable',
        key: 'middleTable',
        render: text => (+text === 1 ? '是' : '否'),
      },
      {
        ...
        ellipsis: true,
        render: val => (
          <Tooltip placement="topLeft" title={this.getOrgNameByorgCode(val)}>
      ...
      },
      {
        ...
        dataIndex: 'sysId',
        key: 'sysId',
        render: val => this.getNameByCode(val, 'attributionSystem'),
      },
      {
        title: '接口通用地址',
    ...
    const fieldColumns = [
      ...
      },
      {
        ...
        editable: true,
        width: 100,
        render: text => (text !== undefined ? (+text === 1 ? '是' : '') : ''),
      }, // 主键的否，前端展示空
      {
        title: '必输项',
        dataIndex: 'required',
        ...
        editable: true,
        width: 100,
        render: text => (text !== undefined ? (+text === 1 ? '是' : '否') : ''),
      },
          ...
          return +record._id === 9999 ? (
            ...
            <span>
              ...
              <EditableContext.Consumer>
                {form => (
                  <a onClick={() => this.save(form, record._id, record)} style={{ marginRight: 8 }}>
                    保存
                  </a>
                )}
              </EditableContext.Consumer>
    ...
    const personalityColumns = [
      ...
      },
      {
        ...
        editable: true,
        width: 150,
        render: val => this.getPersonalityNameByCode(val, 'configName'),
      },
      {
        title: '值',
        ...
        width: 150,
        render: (val, record) =>
          this.getPersonalityNameByCode(val, 'configValue', record.configName),
      },
      {
        title: '说明',
      ...
      },
      {
        ...
        render: (text, record, index) => {
          ...
          return +record.id === 9999 ? (
            <a onClick={this.vipHandleAdd}>新增</a>
          ) : editable ? (
            <span>
              ...
              <EditableContext.Consumer>
                {form => (
                  <a
                    onClick={() => this.vipSave(form, record.id, record)}
                    style={{ marginRight: 8 }}
                  >
                    保存
                  </a>
            ...
            <span>
              <a
                disabled={vipEditingKey !== ''}
                onClick={() => this.vipEdit(record.id, record.configName)}
              >
                修改
              </a>
              <Divider type="vertical" />
    ...
    const elementsColumns = [
      ...
      },
      {
        ...
        fixed: 'left',
        width: 100,
        render: val => this.getNameByCode(val, 'plmSysPEDataGrading'),
      },
      {
        title: '数据主体',
        dataIndex: 'dataSubject',
        key: 'dataSubject',
        render: val => this.getNameByCode(val, 'plmSysPEDataSubject'),
      },
      {
        title: '一级分类',
        dataIndex: 'dataThemeLevelOne',
        key: 'dataThemeLevelOne',
        render: val => this.getNameByCode(val, 'plmSysPESubjectTopCategory'),
      },
      {
        title: '二级分类',
        dataIndex: 'dataThemeLevelTwo',
        key: 'dataThemeLevelTwo',
        render: val => this.getNameByCode(val, 'plmSysPESubjectSecondCategory'),
      },
      {
        title: '三级分类',
        dataIndex: 'dataThemeLevelThree',
        key: 'dataThemeLevelThree',
        render: val => this.getNameByCode(val, 'plmSysPESubjectThirdCategory'),
      },
      {
        title: '业务场景',
        dataIndex: 'bizScene',
        key: 'bizScene',
        render: val => this.getNameByCode(val, 'plmSysPEBusinessScenario'),
      },
      {
        title: '业务分类',
        dataIndex: 'bizClass',
        key: 'bizClass',
        render: val => this.getNameByCode(val, 'plmSysPEBusinessClassification'),
      },
      {
        title: '组件类型',
        dataIndex: 'widgetType',
        key: 'widgetType',
        render: val => this.getNameByCode(val, 'plmSysPEComponentType'),
      },
      {
        title: '要素变更方式',
        dataIndex: 'elementChangeType',
        key: 'elementChangeType',
        render: val => this.getNameByCode(val, 'plmSysPEElementChangeWay'),
      },
      {
        title: '是否为系统级别字段',
        dataIndex: 'systemLevel',
        key: 'systemLevel',
        render: val => (val !== undefined ? (+val === 1 ? '是' : '否') : ''),
      },
      {
        title: '排序',
        dataIndex: 'sort',
        key: 'sort',
        render: val => this.getNameByCode(val, 'plmSysPESort'),
      },
      {
        title: '数据来源',
        dataIndex: 'source',
        key: 'source',
        render: val => this.getNameByCode(val, 'plmSysPEDataSource'),
      },
      {
        title: '数据推送(系统)',
        dataIndex: 'pushSystem',
        key: 'pushSystem',
        render: val => this.getNameByCode(val, 'plmSysPEDataPushSystem'),
      },
      {
        title: '数据推送(场景)',
        dataIndex: 'pushScene',
        key: 'pushScene',
        render: val => this.getNameByCode(val, 'plmSysPEScene'),
      },
      ...
      { title: '提示语', dataIndex: 'tip', key: 'tip' },
      {
        ...
        dataIndex: 'orgId',
        key: 'orgId',
        render: val => this.getOrgNameByorgCode(val),
      },
      {
        title: '归属系统',
        dataIndex: 'sysId',
    ...
    const rowSelection = {
      onChange: (selectedRowKeys, selectedRows) => {
        this.setState({
          selectedRowKeys,
          selectedRows,
          createTableBtn: selectedRowKeys.length > 0,
          initBtn: selectedRowKeys.length === 1,
          optBtn: selectedRowKeys.length > 0,
        });
      },
    };
      ...
      <>
          ...
          extra={
            <>
              ...
              <Button
                style={{ marginRight: 8 }}
                disabled={!initBtn}
                onClick={() => this.setfieldModal('批量初始化', true, {})}
              >
                批量初始化
              </Button>
              <Button style={{ marginRight: 8 }} disabled={!optBtn} onClick={this.exportItems}>
                导出所选项
              </Button>
              <Button style={{ marginRight: 8 }} onClick={this.exportAll}>
                导出全部
              </Button>
              {/* <Upload
              ...
              <Button type="primary" onClick={() => this.setModal('新增', false, {})}>
                新增
              </Button>
            </>
          }
        ...
        >
          ...
          <div style={{ textAlign: 'right', marginBottom: 20, marginTop: 20 }}>
            <Button style={{ marginRight: 10 }} onClick={this.detailsCancel}>
              取消
            </Button>
          </div>
        </Modal>
        ...
        >
          <Row style={{ marginBottom: 10 }}>
            <Col md={16}>
              表名：
              {isBatch
                ? selectedRows[0].masterTableName
                : `${record.masterTableName}（${record.masterTableDesc}）`}
            </Col>
            <Col md={8} style={{ textAlign: 'right' }}>
              <Button
                style={{ marginRight: 8 }}
                onClick={this.setElementsModal}
                loading={fieldLoading}
              >
                业务要素匹配
              </Button>
              {!sysFieldBtn && (
                <Button onClick={this.createField} loading={fieldLoading}>
                  生成系统级别字段
                </Button>
              )}
            </Col>
          ...
          <div style={{ textAlign: 'right', marginBottom: 20, marginTop: 20 }}>
            ...
            <Button style={{ marginRight: 10 }} onClick={this.fieldCancel}>
              取消
            </Button>
            <Button
              type="primary"
              style={{ marginRight: 10 }}
              onClick={this.checkColumn}
              loading={fieldLoading}
            >
              确定
            </Button>
        ...
        >
          ...
          <div style={{ textAlign: 'right', marginBottom: 20, marginTop: 20 }}>
            <Button style={{ marginRight: 10 }} onClick={this.vipModalCancel}>
              取消
            </Button>
            <Button
              type="primary"
              style={{ marginRight: 10 }}
              onClick={this.vipSubmit}
              loading={vipLoading}
            >
              确定
            </Button>
        ...
        >
          <Spin spinning={elementsLoading}>
            <Row style={{ display: isForm ? 'block' : 'none', marginBottom: 10 }}>
              <Col span={16}>
                <Search
                  ...
                  placeholder="请输入"
                  onSearch={val => this.elementsSearch(val)}
                />
            ...
            <Form {...layout} style={{ display: isForm ? 'none' : 'block', marginBottom: 10 }}>
              <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
                <Col md={6}>
                  <Form.Item label="归属机构:" {...formItemLayout}>
                    {getFieldDecorator('orgId', {
                      // 但单击批量初始化时，默认值为selectedRows[0]?.orgId
                ...
                <Col md={6}>
                  <Form.Item label="归属系统:" {...formItemLayout}>
                    {getFieldDecorator('sysId', {
                      // 但单击批量初始化时，默认值为selectedRows[0]?.sysId
                ...
                <Col md={6}>
                  <Form.Item label="英文业务属性:" {...formItemLayout}>
                    {getFieldDecorator('attributeName')(<Input placeholder="请输入英文业务属性" />)}
                  </Form.Item>
                </Col>
                <Col md={6}>
                  <Form.Item label="中文业务属性:" {...formItemLayout}>
                    {getFieldDecorator('bizDataName')(<Input placeholder="请输入中文业务属性" />)}
                  </Form.Item>
                </Col>
                <Col md={6}>
                  <Form.Item label="数据分级:" {...formItemLayout}>
                    {getFieldDecorator('dataLevel')(
                      <Select placeholder="请选择数据分级" showSearch optionFilterProp="children">
                        {dropDownData?.plmSysPEDataGrading?.length > 0 &&
                          dropDownData.plmSysPEDataGrading?.map(item => (
                            <Option key={item.code} value={item.code}>
                              {item.name}
                            </Option>
                          ))}
            ...
            <div style={{ textAlign: 'right', marginBottom: 20, marginTop: 20 }}>
              ...
              <Button style={{ marginRight: 10 }} onClick={this.elementsCancel}>
                取消
              </Button>
              <Button
                disabled={!elementsRows.length > 0}
                type="primary"
                style={{ marginRight: 10 }}
                onClick={this.elementsSave}
              >
                确定
              </Button>
            </div>
        ...
        >
          ...
          <div style={{ textAlign: 'right', marginBottom: 20, marginTop: 20 }}>
            <Button style={{ marginRight: 10 }} onClick={this.tableModalCancel}>
              取消
            </Button>
            <Button
              type="primary"
...
class EditableCell extends React.Component {
  ...
  selectChange = (val, option, setFieldsValue) => {
    ...
    isRequired = !!precision; // 当remarks有值时，精度可编辑
    selNumber++;
  ...
  getInput = setFieldsValue => {
    ...
    if (['columnName', 'columnNameDesc', 'commInterface'].includes(inputType)) {
      return <Input onChange={inputType === 'columnName' ? this.columnNameRepeatCheck : null} />;
    }
    ...
    if (inputType === 'columnType') {
      return (
        <Select
          style={{ width: '100%' }}
          onChange={(val, option) => this.selectChange(val, option, setFieldsValue)}
          showSearch
          optionFilterProp="children"
        >
          {dataSourceType === 'ORACLE'
            ? oracleOption?.map(item => (
                <Option key={item.code} value={item.code} remarks={item.remarks}>
                  {item.code}
                </Option>
              ))
  ...
  getInput = setFieldsValue => {
    ...
    if (inputType === 'configName') {
      return (
        <Select
          style={{ width: '100%' }}
          onChange={val => this.configNameChange(val, setFieldsValue)}
          showSearch
          optionFilterProp="children"
        >
          {columnConfigTypeData?.map(item => (
    ...
    if (inputType === 'configValue') {
      return (
        <Select
          style={{ width: '100%' }}
          onChange={val => this.configValueChange(val, setFieldsValue)}
          showSearch
          optionFilterProp="children"
        >
          {optionData?.map(item => (
        ...
        {editing ? (
          <FormItem style={{ margin: 0 }}>
            {getFieldDecorator(dataIndex, {
              rules: [
                {
                  required: ['configName', 'configValue'].includes(dataIndex),
                  message: '请正确填写',
                },
              ],
...
const WrappedSingleForm = errorBoundary(
  linkHoc()(
    Form.create()(
      connect(({ tableStructure, dataSource, productElements }) => ({
        tableStructure,
        dataSource,
        productElements,
      }))(Index),
