...
const Index = (props)=>{
  ...
  let formItemData=[//高级搜索
    ...
    },
    {
      ...
      config: {mode: 'multiple'},
      option: queryInfoByList?.rpOrgType,
    },
    {
      name: 'orgProperty',
      label: '机构性质',
      ...
      readSet: {name: 'name', code: 'code'},
      config: {mode: 'multiple'},
      option: rpOrgProperty?.rpOrgProperty,
    },
  ]
  useEffect(() => {
    //主页面数据
    ...
    //高级搜索下拉数据
    handleGetSelectOptions('TRpProductOrg','','rpOrgType','rpOrgProperty')
  }, []);
  ...
  const handleCanCheck = record => {
    return router.push(
      `/dynamicPage/pages/产品相关服务机构/4028e9c17f76cf7c017f940fb685000c/查看?type=view&id=${record.id}&secondary=true`
    );
  }
  // 新增
  ...
  const handleCanDelete = record => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        request(`/yss-contract-server/RpProductOrg/deleteByIds`,
  ...
  const checking = async (record) => {
    ...
    request(`/yss-contract-server/RpProductOrg/isCheck`,
  ...
  const handleGetListFetch = ( publicTas,pageSize,pageNum,field,direction,formData) => {
    dispatch({
      type: 'productServiceOrganizations/handleListFetch',
      payload: {
        publicTas,
        pageSize,
        pageNum,
      ...
      val:{url:`/yss-contract-server/RpProductOrg/queryByPage`,method:'POST',},
      callback: (res) => {
  ...
  return(
    <>
      ...
      <List pageCode="productServiceOrganizations"
            // pageContainerProps={{
            //   breadcrumb: [{ title: '基本信息管理', url: '' },{ title: '产品相关服务机构', url: '' }]
            // }}
            ...
            extra={<>
              ...
              <ImportData data={props}
                          url={'/ams/yss-contract-server/RpProductOrg/excel/import'}
              ...
              {templateDownload('/ams/yss-contract-server/RpOrgInfo/excel/downloadFile?fileName=产品相关机构服务导入模板',
                '产品相关机构服务导入模板.xlsx')}
            </>}
            tableList={
              <>
...
const WrappedIndexForm = errorBoundary(
  linkHoc()(
    Form.create()(
      connect(({ productServiceOrganizations, loading, publicModel: { publicTas } }) => ({
        productServiceOrganizations,
        publicTas,
        listLoading: loading.effects['productServiceOrganizations/handleListFetch'],
      }))(Index),
    ),
  ),
);
