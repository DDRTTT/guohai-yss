...
const Index = (props)=>{
  ...
  useEffect(() => {
    ...
    //高级搜索下拉数据
    handleGetSelectOptions('TRpManagerOffice','')
    return () => {}
  }, []);
  ...
  const handleCanCheck = record => {
    return router.push(
      `/dynamicPage/pages/产品经理人任职情况/4028e9c17f76cf7c017fd85401c90018/查看?type=view&id=${record.id}&secondary=true`
    );
  }
  // 新增
  const handleAdd = (record) => {
    return router.push(`/dynamicPage/pages/产品经理人任职情况管理/4028e9c17f76cf7c017f89ce1a830001/新增?secondary=true`);
  };
  // 修改
  const handleCanUpdate = record => {
    router.push(`/dynamicPage/pages/产品经理人任职情况/4028e9c17f76cf7c017fdc1169c80019/修改?id=${record.id}&secondary=true`)
  };
  // 删除
  const handleCanDelete = record => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        request(`/yss-contract-server/RpManagerOffice/deleteByIds`,
  ...
  const checking = async (record) => {
    ...
    request(`/yss-contract-server/RpManagerOffice/isCheck`,
  ...
  const handleGetListFetch = ( publicTas,pageSize,pageNum,field,direction,formData) => {
    dispatch({
      type: 'positionProductManager/handleListFetch',
      payload: {
        publicTas,
        pageSize,
        pageNum,
      ...
      val:{url:`/yss-contract-server/RpManagerOffice/queryByPage`,method:'POST',},
      callback: (res) => {
  ...
  const handleGetSelectOptions = (value1,value2) => {
    ...
    dispatch({
      type: 'positionProductManager/handleSearch',
      payload: { coreModule:value1||'' },
      val:{url:`/yss-contract-server/RpProduct/queryProductInfo`,method:'GET',},
    });
    dispatch({
      type: 'positionProductManager/getProductManagerInfoV',
  ...
  return(
    <>
      {/*{listLoading?<Loading/>:''}*/}
      <List pageCode="positionProductManager"
            // pageContainerProps={{
            //   breadcrumb: [{ title: '基本信息管理', url: '' },{ title: '产品经理人任职情况', url: '' }]
            // }}
            ...
            extra={<>
              ...
              <ImportData data={props} method={'POST'}
                          url={'/ams/yss-contract-server/RpManagerOffice/excel/import'}
              ...
              <ExportData data={props} selectedRows={selectedRows} method={'POST'}
                          url={'/ams/yss-contract-server/RpManagerOffice/export'}
              />
              <ExportAll data={props} url={'/ams/yss-contract-server/RpManagerOffice/exportExcelAll'}/>
              {templateDownload('/ams/yss-contract-server/RpOrgInfo/excel/downloadFile?fileName=产品经理人任职情况导入模板','产品经理人任职情况导入模板.xlsx')}
            </>}
            tableList={
              <>
                {tableCom(columns)}
...
const WrappedIndexForm = errorBoundary(
  linkHoc()(
    Form.create()(
      connect(({ positionProductManager, loading, publicModel: { publicTas } }) => ({
        positionProductManager,
        publicTas,
        listLoading: loading.effects['positionProductManager/handleListFetch'],
      }))(Index),
    ),
  ),
);
