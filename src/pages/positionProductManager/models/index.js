...
export default {
  namespace: 'positionProductManager',
  state: {
  ...
  effects: {
    ...
    *getProductManagerInfoV({payload, val, callback}, {call, put}) {
      const response = yield call(handSearchAPI, payload, val);
      if (response && response.status === 200) {
        yield put({
          type: 'getProductManagerInfo',
          payload: response.data
        });
        if (callback) callback(response);
      } else {
  ...
  reducers: {
    ...
    getProductManagerInfo(state, { payload }) {
      return {
        ...state,
        getProductManagerInfo: payload,
      };
    },
  },
};
