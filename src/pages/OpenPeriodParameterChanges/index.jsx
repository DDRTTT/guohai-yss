...
const Index = ({
  ...
  dispatch,
  listLoading,
  OpenPeriodParameterChanges: { dicts, proNameAndCodeData, riseOverTableData },
  publicTas,
}) => {
  ...
  const handleGetListData = () => {
    handleGetDataObj();
    dispatch({
      type: 'OpenPeriodParameterChanges/fetch',
  ...
  const handleGetSelectOptions = () => {
    dispatch({
      type: 'OpenPeriodParameterChanges/getDicts',
  ...
  const handleGetProTypeData = () => {
    dispatch({
      type: 'OpenPeriodParameterChanges/getProTypeFunc',
    });
  };
  ...
  const handleGetProNameAndCode = () => {
    dispatch({
      type: 'OpenPeriodParameterChanges/getProNameAndCodeFunc',
    });
  };
  ...
  const handleGetInvestmentManager = () => {
    dispatch({
      type: 'OpenPeriodParameterChanges/getInvestmentManagerFunc',
    });
  };
   ...
   */
  const handleGoAdd = () => {
    fnLink('OpenPeriodParameterChanges:link', '');
    // dispatch(
    //   routerRedux.push({
  ...
  // 发起流程按钮
  const operations = (
    <Action code="OpenPeriodParameterChanges:link">
  ...
  const handleCanUpdate = record => {
    fnLink(
      'OpenPeriodParameterChanges:update',
  ...
  const handleCanCopy = record => {
    fnLink('OpenPeriodParameterChanges:copy', `?processInstId=${record.processInstanceId}`);
  };

  // 提交
  const handleCanSubmit = record => {
    fnLink(
      'OpenPeriodParameterChanges:commit',
      `?id=${record.id}&proCode=${record.proCode}&processInstanceId=${record.processInstanceId}&processInstId=${record.processInstanceId}`,
    );
  };
  ...
  const handleCanBackOut = record => {
    ...
    confirm({
      ...
      onOk() {
        dispatch({
          type: 'OpenPeriodParameterChanges/getRevokeFunc',
  ...
  const handleCanDelete = record => {
    ...
    confirm({
      ...
      onOk() {
        dispatch({
          type: 'OpenPeriodParameterChanges/getDeleteFunc',
  ...
  const handleAddButtonUpdate = record => {
    return (
      <Action code="OpenPeriodParameterChanges:update">
  ...
  const handleAddButtonCopy = record => {
    return (
      <Action code="OpenPeriodParameterChanges:copy">
  ...
  const handleAddButtonCommit = record => {
    return (
      <Action code="OpenPeriodParameterChanges:commit">
  ...
  const handleAddButtonCheck = record => {
    return (
      <Action code="OpenPeriodParameterChanges:check">
  ...
  const handleAddButtonTransferHistory = record => {
    return (
      <Action code="OpenPeriodParameterChanges:transferHistory">
        <Button
          type="link"
  ...
  const handleAddButtonBackOut = record => {
    return (
      <Action code="OpenPeriodParameterChanges:backOut">
  ...
  const handleAddButtonDelete = record => {
    return (
      <Action code="OpenPeriodParameterChanges:delete">
  ...
  const handleAddButtonDetails = record => {
    return (
      <Action code="OpenPeriodParameterChanges:details">
  ...
  const handlerBatchSubmit = () => {
    dispatch({
      type: 'OpenPeriodParameterChanges/handleGetBatchCommitFunc',
      payload: batchData,
      callback: () => {
        handleGetListData();
      },
  ...
  return (
    <>
      <List
        pageCode="OpenPeriodParameterChanges"
...
const WrappedIndexForm = errorBoundary(
  linkHoc()(
    Form.create()(
      connect(({ OpenPeriodParameterChanges, loading, publicModel: { publicTas } }) => ({
        OpenPeriodParameterChanges,
        listLoading: loading.effects['OpenPeriodParameterChanges/fetch'],
        publicTas,
      }))(Index),
    ),
  ),
