...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/业绩报酬信息/4028e7b675780970017603392d9e0050/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/业绩报酬信息/4028e7b6757809700175f8f826750043/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '是否提取业绩报酬',
      dataIndex: 'extractRemuneration',
      ...tableRender,
    },
    {
      title: '计提时点',
      dataIndex: 'accruedTimePoint',
      ...tableRender,
    },
    {
      title: '计提基准',
      dataIndex: 'accruedDatum',
      ...tableRender,
    },
    {
      title: '计提比例(%)',
      dataIndex: 'accruedProportion',
      ...moneyRender,
    },
    {
      title: '操作',
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '业绩报酬信息')}
