...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/费率信息/4028e7b676216e1b01762c009687000c/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}&isAddKey=yes`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/费率信息/4028e7b676216e1b017646e74a14002a/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}&isAddKey=yes`;
      router.push(url);
    }
  };
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '费率信息')}
