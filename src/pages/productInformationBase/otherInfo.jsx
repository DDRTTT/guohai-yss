...
import { handleAddTooltip } from '@/pages/productBillboard/baseFunc';
...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/其他信息/4028e7b67578097001760339af360051/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}&id=${record.id}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/其他信息/4028e7b676216e1b017645f484360027/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}&id=${record.id}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '合同变更的方式',
      dataIndex: 'contractChange',
      ...tableRowConfig,
      sorter: false,
    },
    {
      title: '合同变更的方式(其他方式)',
      dataIndex: 'contractChangeOther',
      render: (_, record) => {
        return handleAddTooltip(
          record?.contractChange?.indexOf('其他') !== -1 ? record.contractChangeOther : '',
        );
      },
    },
    {
    ...
    {
      title: '合同终止情形(附加情形)',
      dataIndex: 'endOfContractOther',
      render: (_, record) => {
        return handleAddTooltip(
          record?.endOfContract?.indexOf('其他') !== -1 ? record.endOfContractOther : '',
        );
      },
    },
    {
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '其他信息')}
