...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/清盘信息/4028e7b676216e1b01763c0b9d060023/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/清盘信息/4028e7b676216e1b01763c1a00d20026/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '清盘原因',
      dataIndex: 'clearReason',
      ...tableRender,
    },
    {
      title: '是否清算完成',
      dataIndex: 'clearComplete',
      ...tableRender,
    },
    {
      title: '操作',
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '清盘信息')}
