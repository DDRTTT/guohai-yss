...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/服务机构信息/4028e7b6757809700176032312a9004c/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/服务机构信息/4028e7b6757809700175f8d2485b0041/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '境外投资顾问机构',
      dataIndex: 'overseaInvest',
      ...tableRender,
    },
    {
      title: '境外托管人',
      dataIndex: 'overseaTrustee',
      ...tableRender,
    },
    {
      title: '操作',
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '服务机构信息')}
