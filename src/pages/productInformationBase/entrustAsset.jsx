...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/委托财产信息/4028e7b675780970017603344b46004e/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/委托财产信息/4028e7b6757809700175f900aa350044/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '委托资产类型',
      dataIndex: 'typeOfEntrustedAsset',
      ...tableRender,
    },
    {
      title: '委托财产金额',
    ...
    {
      title: '委托人是否为管理人董事、监事、从业人员及其配偶',
      dataIndex: 'consignorTypeSome',
      ...tableRender,
    },
    {
      title: '操作',
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '委托财产信息')}
