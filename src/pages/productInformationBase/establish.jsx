...
const Index = ({ dispatch, listLoading, productInformationBase: { productTableInfo } }) => {
  ...
  const handleGoCheck = (record, val) => {
    console.log(record);
    if (val) {
      const url = `/dynamicPage/pages/成立信息/4028e7b6757809700176033c31970054/查看?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    } else {
      const url = `/dynamicPage/pages/成立信息/4028e7b6757809700175f8428072003b/修改?proCode=${record.proCode}&type=${name}&updateDataType=${name}`;
      router.push(url);
    }
  };
  ...
  const columns = [
    ...
    {
      title: '备案代码',
      dataIndex: 'proRecodeCode',
      ...tableRender,
    },
    {
      title: '初始备案日期',
      dataIndex: 'beginRecodeDate',
      ...tableRender,
    },
    {
      title: '完成备案日期',
      dataIndex: 'completeRecodeDate',
      ...tableRender,
    },
    {
      title: '操作',
  ...
  return (
    <>
      {handleAddHeard(handleCanSearch, 1, '产品生命周期', '产品信息要素', '成立信息')}
