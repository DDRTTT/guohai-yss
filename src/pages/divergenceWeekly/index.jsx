...
@Form.create()
class DiverGenceWeekly extends Component {
  state = {
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          if (this.state.taskTypeCode === 'T001_1' && record.circulateFlag === '0') {
            ...
            if (this.state.taskTypeCode === 'T001_1' || this.state.taskTypeCode === 'T001_4') {
              switch (record.operStatus) {
                case 'S001_1':
                  content = (
                    <>
                      <Action code="divergenceWeekly:update">
                        <Button
                          type="link"
                          size="small"
                      ...
                      <Action code="divergenceWeekly:copy">
                        <Button
                          type="link"
                          size="small"
                      ...
                      <Action code="divergenceWeekly:commit">
                        <Button
                          type="link"
                          size="small"
                      ...
                      <Action code="divergenceWeekly:deleteApi">
                        <Button
                          type="link"
                          size="small"
                          onClick={() => {
                ...
                case 'S001_2':
                  content = (
                    <>
                      <Action code="divergenceWeekly:check">
                        <Button
                          type="link"
                          size="small"
                      ...
                      <Action code="divergenceWeekly:transferHistory">
                        <Button
                          type="link"
                          size="small"
            ...
            content = (
              <>
                <Action code="divergenceWeekly:details">
  ...
  getDictList() {
    ...
    dispatch({
      type: `diverGenceWeekly/queryCriteria`,
      payload: {
        codeList,
      },
    });
  ...
  handleGetTableList = () => {
    ...
    dispatch({
      type: 'diverGenceWeekly/searchTableData',
      payload,
    });
  };
   ...
   */
  editTask() {
    this.props.fnLink('divergenceWeekly:link', '');
  }
  ...
  setOperations = () => {
    return (
      <Action code="divergenceWeekly:link">
  ...
  revokeBtn = record => {
    if (record.operStatus === 'S001_2' && record.revoke && record.revoke * 1 === 1) {
      return (
        <Action code="divergenceWeekly:backOut">
          <Button
            type="link"
            size="small"
            onClick={() => {
  ...
  dealTask(record, mark) {
    ...
    if (mark === 'handle') {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          dispatch({
            type: 'diverGenceWeekly/getRevokeFunc',
      ...
      Modal.confirm({
        ...
        onOk: () => {
          dispatch({
            type: 'diverGenceWeekly/getDeleteFunc',
            payload: record.id,
          }).then(data => {
            if (data) {
              this.handleGetTableList();
      ...
      fnLink('divergenceWeekly:copy', `?processInstId=${record.processInstanceId}`);
    }
  }
  ...
  handlerBatchSubmit = () => {
    ...
    this.props
      .dispatch({
        type: 'diverGenceWeekly/getBatchSubmitByProCodeReq',
        payload: proCodes,
      })
      .then(res => {
  ...
  render() {
    const {
      diverGenceWeekly: { dataList, statusList },
      loading,
    } = this.props;
        ...
        <>
          <Table
            pagination={{
              ...
              total: this.props.diverGenceWeekly.total,
...
const WrappedSingleForm = errorBoundary(
  linkHoc()(
    Form.create()(
      connect(({ diverGenceWeekly, loading, publicModel: { publicTas } }) => ({
        diverGenceWeekly,
        publicTas,
        loading: loading.effects['diverGenceWeekly/searchTableData'],
      }))(DiverGenceWeekly),
    ),
  ),
);
