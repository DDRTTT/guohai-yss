...
class FileList extends React.Component {
          ...
          const content = (
            <>
              {this.props.falg !== 'details' ? (
                <a style={{ marginRight: '10px' }} onClick={() => this.downLoadFile(record)}>
                  下载
                </a>
              ) : null}
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.falg && nextProps.businessArchives) {
  ...
  render() {
    ...
    return (
        <Card
          title='机构文件'
          extra={[
            <Button onClick={() => this.selectAll()} style={{ marginRight: '20px' }}>
              批量下载
            </Button>,
