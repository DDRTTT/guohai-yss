...
var RangeDownloader = (function() {
  var RangeDownloader = function(param) {
    ...
    this.body = param.body || null;
    if (!this.url) {
      throw Error('Invaild url.');
    }
    ...
    try {
      fetch(this.url, {
        ...
        .then(function(resp) {
          ...
          _self.totalSize =
            _self.totalSize == NaN
              ? parseInt(resp.headers.get('Content-Range').split('/')[1])
              : _self.totalSize;
          _self.running = true;
          _self.fileName = resp.headers.get('fname') || '';
