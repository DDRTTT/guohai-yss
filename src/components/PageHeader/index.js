...
export default class PageHeader extends PureComponent {
  ...
  render() {
    ...
    const {
      ...
      activeTabKey,home,father_url
    } = this.props;


    const clsString = classNames(home == "home" ? styles.pageHeader_home : styles.pageHeader, className);
    let breadcrumb;
    ...
    if (breadcrumbList && breadcrumbList.length) {
      ...
      const  All_URL=(father_url !== null && father_url !== undefined) ?
        breadcrumbItems.splice(0,2,(
          <Breadcrumb.Item key="home">
            {createElement(linkElement, {
              [linkElement === 'a' ? 'href' : 'to']: father_url,
            }, getBreadcrumb(breadcrumbNameMap, father_url).name )}
          </Breadcrumb.Item>
        )):breadcrumbItems ;
