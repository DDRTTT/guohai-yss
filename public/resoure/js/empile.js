(function(win, doc) {
  ...
  function getTransitionEndName() {
    var div = doc.createElement('div'),
      transitionendName = '';
    if (
      'onwebkittransitionend' in div &&
      !('ontransitionend' in div)
    ) {
  ...
  function afterSlideTo() {
    ...
    empile.afterSlideTo.transitionEndTimer = setTimeout(function() {
      ...
      empile.trigger(EMPILE_HOOKS.clickDotAfter, activeIndex);
    }, interval);
  }
    ...
    getClickFun: function() {
      ...
      var wrapperClickFn = function(ev) {
        ...
        if (willTar === navigation.prevEl) {
          empile.slideToPrev();
          empile.trigger(EMPILE_HOOKS.clickPrevBtn, ev, activeIndex);
        } else if (willTar === navigation.nextEl) {
          empile.slideToNext();
          empile.trigger(EMPILE_HOOKS.clickNextBtn, ev, activeIndex);
